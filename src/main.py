...
def remove_citation_sections(text: str) -> str:
    ...
    for line in lines:
        ...
        if (re.match(r'^\s*\[?\d+\]?\s*.*https?://', line) or 
            re.match(r'^\s*\d+[\.\)]\s*.*https?://', line) or
            (line.strip().startswith('http') and len(line.split()) <= 10)):
...
@click.option('--clipboard-input', '-ci', is_flag=True, default=False,
              help='Force read from clipboard')
@click.option('--clipboard-output', '-co', is_flag=True, default=False,
              help='Force write to clipboard')
def main(input_file: Optional[str], output_file: Optional[str], 
         clipboard_input: bool, clipboard_output: bool) -> None:
    ...
    if input_file and clipboard_input:
        click.echo("Error: Cannot specify both --input-file and --clipboard-input", err=True)
        sys.exit(1)
    
    if output_file and clipboard_output:
        click.echo("Error: Cannot specify both --output-file and --clipboard-output", err=True)
        sys.exit(1)
